...
# Agregar botón flotante para mostrar sidebar cuando está oculto
st.markdown("""
<div id="sidebar-toggle-btn" class="sidebar-toggle-button">☰</div>
<script>
(function() {
    ...
    function checkSidebarVisibility() {
        ...
        if (sidebar) {
            ...
            const hasWidth = sidebar.offsetWidth > 0;
            
            // Si el sidebar no es visible, mostrar el botón
            if (!isDisplayed || !hasWidth) {
                toggleBtn.style.display = 'flex';
            } else {
                toggleBtn.style.display = 'none';
            }
        } else {
            // Si no existe el sidebar, mostrar el botón
            toggleBtn.style.display = 'flex';
        }
    }
    
    // Verificar visibilidad al cargar y después de un delay
    ...
    // Hacer clic en el botón de menú cuando se hace clic en el botón flotante
    toggleBtn.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        // Buscar el botón de menú de Streamlit de varias formas
        let menuButton = null;
        
        // Método 1: Buscar por data-testid
        menuButton = document.querySelector('[data-testid="stSidebar"] button');
        
        // Método 2: Buscar por aria-label
        if (!menuButton) {
            menuButton = document.querySelector('button[aria-label*="sidebar"], button[aria-label*="menu"]');
        }
        
        // Método 3: Buscar botones en la parte superior izquierda
        if (!menuButton) {
            const allButtons = document.querySelectorAll('button');
            for (let btn of allButtons) {
                const rect = btn.getBoundingClientRect();
                if (rect.top < 60 && rect.left < 150 && rect.width < 100) {
                    menuButton = btn;
                    break;
                }
            }
        ...
        // Método 4: Buscar el primer botón en el header
        if (!menuButton) {
            const header = document.querySelector('header, [data-testid="stHeader"]');
            if (header) {
                menuButton = header.querySelector('button');
            }
        }
        ...
        if (menuButton) {
            ...
            // Si no encontramos el botón, intentar abrir el sidebar directamente
            const sidebar = document.querySelector('[data-testid="stSidebar"]');
            if (sidebar) {
